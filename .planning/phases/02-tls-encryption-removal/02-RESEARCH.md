...
1. **Should TCPSocketFactory be removed entirely or just cleaned?**
   - RESOLVED: Clean TCPSocketFactory — remove SecurityLevel dispatch and SecureSocket references, keep file in build (Plan 02-01 action step 5). Verified: no code outside net/ creates TCPSocketFactory directly.

2. **Should the Settings::Security struct be removed entirely?**
   - RESOLVED: Remove struct entirely — all 4 keys are TLS-specific, QSettings ignores orphan keys (Plan 02-03 action step A).
